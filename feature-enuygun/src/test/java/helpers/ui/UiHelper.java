...
public class UiHelper {
    ...
    public boolean waitElementIfOpenAgain(WebElement webElement) throws Exception {
                independentWait(4);
                logger.info("Wait " + webElement);
        return true;
    }
    ...
    public boolean selectElement(WebElement webElement) throws Exception {
        // Bu method zorla tıklama yapmak için kullanılır
        try {
            Select dropdown = new Select(webElement);
            if (isVisible(webElement)) {

                dropdown.selectByValue("webElement");
                logger.info("UiHelper - selected " + webElement);
            }
        } catch (Exception e) {
            logger.error("\t The method causing the error : UiHelper - selectElement." + webElement);
            throw new Exception(e.getMessage());
        }
        return true;
    }
        ...
        try {
            switch (action) {
                case "click":
                    ...
                    break;
                case "waitIfOpenAgain":
                    waitElementIfOpenAgain(webElement);
                    break;
                case "selectElement":
